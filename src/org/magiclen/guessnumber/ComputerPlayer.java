...
public final class ComputerPlayer extends Player {
     ...
     */
    public ComputerPlayer(String inputName, boolean isAsker) {
     ...
     * @param inputName 電腦玩家的名稱
     */
    public ComputerPlayer(String inputName) { //建構子。只傳入電腦玩家的名稱
	setName(inputName); //設定名稱
	creatQuestion(); //出題
	creatForm(); //建立表格
	setGuessTimes(0); //已猜次數歸零
     ...
     */
    @Override //覆寫
    public void talk(Talk sentence) { //
	switch (sentence) {
	    case START:
     ...
     */
    private void setCorrectAnswer(int intputCorrectAnswer) {
	/*
	 * 判斷輸入值的正確性
	 * 1.必須為四位數或三位數
	 * 2.數字不能重複
     ...
     */
    private String compareHint(String inputAnswer, String referenceAnswer) {
	int a = 0, b = 0; //儲存A,B數量
	for (int i = 0; i < 4; i++) {
	    for (int k = 0; k < 4; k++) {
